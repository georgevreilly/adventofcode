#!/usr/bin/env python3

from __future__ import annotations

YEAR = 2022
DAY = 5
# https://adventofcode.com/2022/day/5

import argparse
import logging
import os


TEST_DATA = "test_input.txt"
REAL_DATA = "input.txt"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=f"AdventOfCode: {YEAR} Day {DAY}")
    parser.set_defaults(
        input_filename=None,
        real=True,
        verbose=False,
        custom=False,
    )
    group = parser.add_mutually_exclusive_group()
    group.add_argument(
        "--test", "-t", dest="real", action="store_false",
        help=f"Use {TEST_DATA!r} as input_filename")
    group.add_argument(
        "--real", "-r", dest="real", action="store_true",
        help="Use {REAL_DATA!r} as input_filename")

    parser.add_argument(
        "--steps", "-s", type=int,
        help="Number of steps")
    parser.add_argument(
        "--custom", "-C", action="store_true",
        help="Use custom data")
    parser.add_argument(
        "--verbose", "-v", action="store_true",
        help="More verbose logging")
    namespace = parser.parse_args()

    namespace.input_filename = REAL_DATA if namespace.real else TEST_DATA
    log_level = logging.DEBUG if namespace.verbose else logging.INFO
    logging.basicConfig(level=log_level)

    return namespace


def read_data(input_filename: str):
    with open(os.path.join(os.path.dirname(__file__), input_filename)) as f:
        return f.read()


def parse_data(text_data: str) -> list[list[str]]:
    stack_txt, move_txt = text_data.split("\n\n")
    vstacks = []
    for line in stack_txt.split("\n"):
        if not "[" in line:
            break
        items = [line[i] for i in range(1, len(line), 4)]
        logging.debug(items)
        vstacks.append(items)
    stack_count = len(vstacks[0])
    stacks = [[] for _ in range(stack_count)]
    for row in reversed(vstacks):
        for i, c in enumerate(row):
            if c != ' ':
                stacks[i].append(c)
    logging.debug(f"{stacks=}")

    moves = []
    for line in move_txt.split("\n"):
        nums = tuple(int(w) for w in line.split() if w.isdigit())
        if nums:
            moves.append(nums)
    logging.debug(f"{moves=}")

    return stacks, moves


def load_data(namespace) -> list[list[int]]:
    if namespace.custom:
        text_data = ["-1"]
        data = parse_data(text_data)
    else:
        text_data = read_data(namespace.input_filename)
        logging.info("%s", namespace.input_filename)
        logging.debug("%s", text_data)
        data = parse_data(text_data)
        logging.debug("%s", data)
    return data


def grid_to_str(grid: list[list[int]], prefix="") -> str:
    return prefix + ("\n" + prefix).join(
        "".join([str(n) for n in row]) for row in grid) + "\n"


def compute1(stacks: list[Any], moves: list[Any]) -> str:
    for n, source, target in moves:
        for _ in range(n):
            item = stacks[source-1].pop()
            stacks[target-1].append(item)
        # print(f"{n=}, {source=}, {target=}, {stacks=}")

    return "".join(s[-1] for s in stacks)


def compute2(stacks: list[Any], moves: list[Any]) -> str:
    for n, source, target in moves:
        # print(f"{n=}, {source=}, {target=}")
        items = [stacks[source-1].pop() for _ in range(n)]
        stacks[target-1].extend(reversed(items))
        # print(f"\t{stacks=}")

    return "".join(s[-1] for s in stacks)


def main():
    namespace = parse_args()
    stacks, moves = load_data(namespace)
    result1 = compute1(stacks, moves)
    print(f"{result1=}")
    stacks, moves = load_data(namespace)
    result2 = compute2(stacks, moves)
    print(f"{result2=}")


if __name__ == "__main__":
    raise SystemExit(main())
